...
window.addEventListener("load", function () {
    ...
    constructor() {
      ...
      if (savedHeroes && savedHeroes.length > 0) {
        this.heroes = savedHeroes.map(data => new Hero({
          ...
          isDead: data.isDead,
          sprite: {
            image: document.getElementById(data.spriteId || (data.gender === 'male' ? 'hero1' : 'hero2')) as HTMLImageElement,
            x: 0,
            y: 0,
            width: 64,
            height: 64,
      ...
      if (savedWorld) {
        ...
        // Restore food
        savedWorld.food.forEach((fData: any) => {
  ...
  const topStatsPanel = document.getElementById("top-stats-panel") as HTMLDivElement;
  const infoPanel = document.getElementById("info-panel") as HTMLDivElement;

  const updateTopStatsPanel = (totalHeroes: number, totalFamilies: number) => {
    if (topStatsPanel) {
  ...
  const updateInfoPanelLayout = () => {
    if (infoPanel) {
      ...
      const families: { [key: string]: Hero[] } = {};
      game.heroes.forEach((hero, index) => {
        ...
        (hero as any)._originalIndex = index;
      ...
      for (const [fId, members] of Object.entries(families)) {
        ...
        <div class="family-group">
          ...
          ${members.map((hero) => {
          const index = (hero as any)._originalIndex;
    ...
    for (const male of males) {
      ...
      for (const female of sortedFemales) {
        ...
        if (dist === 1) {
          ...
          // Fetch name from API
          async function getRandomName(gender: 'male' | 'female', surname: string): Promise<string> {
            try {
